...
Databron-strategie:
  ...
  2. Lukt dat niet (geen netwerk, rate limit, timeout, 0 resultaten) -> val
     terug op een synthetisch, maar realistisch gemodelleerd historisch
     dataset. Elke terugval wordt gelogd (zie journalctl) zodat je precies
     ziet WAAROM live data niet gebruikt werd, in plaats van dat het stil
     gebeurt.
...
# Zonder NVD API-key is de rate-limit te krap om 36 maanden aan CVE-data op
# te halen binnen een redelijke laadtijd (elke 120-dagen-chunk kost een
# aparte request). Zonder key beperken we live-ophalen daarom tot de meest
# recente periode; oudere maanden worden synthetisch aangevuld als context.
# Met een NVD_API_KEY (hogere rate-limit) wordt de volle HISTORY_MONTHS gebruikt.
LIVE_MONTHS_WITHOUT_KEY = 4
...
# Optionele NVD API-key (gratis: https://nvd.nist.gov/developers/request-an-api-key).
# Zonder key: rate-limit van 5 requests/30s. Met key: 50 requests/30s.
# Instellen via omgevingsvariabele NVD_API_KEY (bv. in de systemd unit).
...
# pagina-load opnieuw tegen de (lage) NVD rate-limit aanloopt.
# TTL in seconden, instelbaar via NVD_CACHE_TTL_SECONDS (default 6 uur).
# ---------------------------------------------------------------------------
_LIVE_CACHE_TTL = int(os.environ.get("NVD_CACHE_TTL_SECONDS", str(6 * 3600)))
_live_cache: dict[str, tuple[float, list[dict]]] = {}

# Begrenst gelijktijdige NVD-calls. Zonder API-key mag er maar 1 request
# tegelijk (5 req/30s totaal is te krap voor parallelisme); met een key mag
# dit hoger, want de limiet is dan 50 req/30s.
_nvd_semaphore = asyncio.Semaphore(3 if NVD_API_KEY else 1)


# NVD API 2.0 staat per request een maximale pubStartDate/pubEndDate-range
# van 120 dagen toe. Een grotere range geeft een 404/400 terug (geen
# duidelijke foutmelding over de oorzaak) — vandaar dat we hier altijd in
# stukken van maximaal 120 dagen opvragen en samenvoegen.
_NVD_MAX_RANGE_DAYS = 120


def _iso_utc(d: date, end_of_day: bool = False) -> str:
    """NVD verwacht volledige ISO 8601 met tijdzone-aanduiding (Z=UTC)."""
    t = "23:59:59.999" if end_of_day else "00:00:00.000"
    return f"{d.isoformat()}T{t}Z"


async def _fetch_nvd_chunk(client: httpx.AsyncClient, keyword: str, start: date, end: date, headers: dict) -> list[dict]:
    params = {
        "keywordSearch": keyword,
        "pubStartDate": _iso_utc(start),
        "pubEndDate": _iso_utc(end, end_of_day=True),
        "resultsPerPage": 2000,
    }
...
async def fetch_live_history(cat_key: str) -> list[dict] | None:
    ...
    Elke terugval wordt gelogd met de reden. Resultaten worden gecached.

    Belangrijk: NVD API 2.0 staat max. 120 dagen per pubStartDate/pubEndDate-
    request toe. HISTORY_MONTHS (36 mnd) wordt daarom in stukken van
    _NVD_MAX_RANGE_DAYS opgehaald en samengevoegd.
    """
    keywords = CATEGORY_KEYWORDS.get(cat_key)
    if not keywords:
        logger.warning("Geen trefwoorden bekend voor categorie '%s' — synthetisch.", cat_key)
        return None

    cached = _live_cache.get(cat_key)
    if cached and (time.monotonic() - cached[0]) < _LIVE_CACHE_TTL:
        logger.info("'%s': live NVD-data uit cache (leeftijd %.0fs).", cat_key, time.monotonic() - cached[0])
        return cached[1]

    today = date.today()
    overall_start = _subtract_months(today, HISTORY_MONTHS)
    keyword = keywords[0]
    headers = {"apiKey": NVD_API_KEY} if NVD_API_KEY else {}

    # Zonder API-key: alleen de recentste maanden live ophalen (zie
    # LIVE_MONTHS_WITHOUT_KEY hierboven) — de rest van de reeks wordt na
    # afloop synthetisch aangevuld zodat de grafiek toch 36 mnd context toont.
    if NVD_API_KEY:
        live_start = overall_start
    else:
        logger.info(
            "'%s': geen NVD_API_KEY ingesteld — live-ophalen beperkt tot laatste %d maanden (rate-limit).",
            cat_key, LIVE_MONTHS_WITHOUT_KEY,
        )
        live_start = _subtract_months(today, LIVE_MONTHS_WITHOUT_KEY)

    # Bouw de lijst van (chunk_start, chunk_end) datumparen, elk <= 120 dagen.
    chunks: list[tuple[date, date]] = []
    cursor = live_start
    while cursor < today:
        chunk_end = min(today, cursor + timedelta(days=_NVD_MAX_RANGE_DAYS - 1))
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                for i, (c_start, c_end) in enumerate(chunks):
                    try:
                        chunk_vulns = await _fetch_nvd_chunk(client, keyword, c_start, c_end, headers)
                        vulns.extend(chunk_vulns)
                    except httpx.HTTPStatusError as e:
                        logger.warning(
                            "'%s': NVD gaf HTTP %s terug voor periode %s..%s — deze chunk overgeslagen. "
                            "(429 = rate-limit; overweeg NVD_API_KEY in te stellen)",
                            cat_key, e.response.status_code, c_start.isoformat(), c_end.isoformat(),
                        )
                    # Kleine pauze tussen chunks binnen dezelfde categorie om
                    # de (lage) rate-limit zonder API-key te respecteren.
                    if i < len(chunks) - 1:
                        await asyncio.sleep(0.5 if NVD_API_KEY else 6.5)
        except httpx.TimeoutException:
            logger.warning("'%s': NVD-call timed out (>15s) — synthetisch.", cat_key)
            return None
        except Exception as e:
            logger.warning("'%s': NVD-call faalde (%s: %s) — synthetisch.", cat_key, type(e).__name__, e)
            return None

    if not vulns:
        logger.info(
            "'%s': NVD gaf 0 resultaten voor keyword '%s' in periode %s..%s — synthetisch.",
            cat_key, keyword, overall_start.isoformat(), today.isoformat(),
        )
        return None
    ...
    if not buckets:
        logger.info("'%s': NVD gaf %d CVE's terug maar geen bruikbare publicatiedatums — synthetisch.", cat_key, len(vulns))
        return None
    ...
    logger.info("'%s': live NVD-data opgehaald en gecached (%d CVE's verwerkt, TTL %ds).", cat_key, len(vulns), _LIVE_CACHE_TTL)
    return series
...
async def predict_all(
    horizon: int = Query(FORECAST_MONTHS_DEFAULT, ge=1, le=36),
    prefer_live: bool = Query(True, description="Probeer live NVD-data (parallel + gecached)"),
):
    ...
    if prefer_live:
        ...
        # begrenst gelijktijdige NVD-calls, dus dit blijft rate-limit-veilig
        # maar is veel sneller dan 8x na elkaar wachten.
        ...
        for cat_key, result in zip(CATEGORIES, fetched):
            if isinstance(result, Exception):
                logger.warning("'%s': onverwachte fout bij live-fetch: %s", cat_key, result)
                continue
    ...
    live_count = sum(1 for r in results if r["source"] == "live-nvd")
    logger.info("predict-all: %d/%d categorieën met live NVD-data, rest synthetisch.", live_count, len(results))
    return results
