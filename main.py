...
# Begrenst gelijktijdige NVD-calls, ook binnen een gather-batch, als extra
# veiligheidsmarge tegen de rate-limit.
...
async def fetch_live_history(cat_key: str) -> list[dict] | None:
    ...
    if not NVD_API_KEY:
        logger.info("'%s': geen NVD_API_KEY ingesteld — lage rate-limit (5 req/30s) van toepassing.", cat_key)
    ...
    async with _nvd_semaphore:
        try:
            ...
            logger.warning(
                "'%s': NVD gaf HTTP %s terug — synthetisch. "
    ...
    if not vulns:
        logger.info(
            ...
            cat_key, keywords[0], start.isoformat(), today.isoformat(),
        )
        return None
